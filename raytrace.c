...
static inline double* tracer(double* rO, double* rD, Object** objects, int reLev, double iorO)
{
          ...
          if (closest_shadow_object == -1 && shadow == 0)
          {
            ...
            switch(objects[bestO]->kind)
            {
               case 0:
                ...
                reflectivity = objects[bestO]->plane.reflectivity;
                refractivity = objects[bestO]->plane.refractivity;
                ior = objects[bestO]->plane.ior;
               break;
               case 1:
                ...
                reflectivity = objects[bestO]->sphere.reflectivity;
            ...
            else
            {
              ...
              else{fang = pow(v3_dot(rDn,vlight),objects[lightI]->light.angular_a0);
            }
          }
...
Pixel* raycast(Object** objects, int pxW, int pxH)
{
  ...
  Pixel* image = malloc(sizeof(Pixel) * pxW * pxH); //Prepare memory for image data
  for (int y = pxH; y >= 0; y -= 1) {
    for (int x = 0; x < pxW; x += 1) {
      ...
      color = tracer(rO, rD, objects, 0, 1.0);
          ...
          image[pxH*(pxH - y) + x].b = (unsigned char)(color[2]);
        ...
        else if (c == ',') // if there is another parameter
        {
            ...
            else
            {
          ...
          else if ((strcmp(key, "ior") == 0))
              {
                ...
                {
                  objects[objectI]->plane.ior = value;
                }
                else if (objects[objectI]->kind == 1)
                {
                  objects[objectI]->sphere.ior = value;
                }
                else
                {
                  fprintf(stderr, "Error: Invalid property, \"%s\", on line number %d.\n", key, line);
            ...
            else if ((strcmp(key, "reflectivity") == 0))
                {
                  ...
                  if (objects[objectI]->kind == 0)
                  {
                    objects[objectI]->plane.reflectivity = value;
                  }
                  else if (objects[objectI]->kind == 1)
                  {
                    objects[objectI]->sphere.reflectivity = value;
                  }
                  else
                  {
                    fprintf(stderr, "Error: Invalid property, \"%s\", on line number %d.\n", key, line);
            ...
            else if ((strcmp(key, "refractivity") == 0))
                {
                  ...
                  if (objects[objectI]->kind == 0)
                  {
                    objects[objectI]->plane.refractivity = value;
                  }
                  else if (objects[objectI]->kind == 1)
                  {
                    objects[objectI]->sphere.refractivity = value;
                  }
                  else
                  {
                    fprintf(stderr, "Error: Invalid property, \"%s\", on line number %d.\n", key, line);
